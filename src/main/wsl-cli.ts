...
function runWsl(args: string[], options: { input?: string } = {}) {
  return spawnSync("wsl.exe", args, { encoding: "utf8", windowsHide: true, input: options.input, timeout: 10_000 });
}

export function listWslDistributions(): WslProbe {
  const result = runWsl(["--list", "--quiet"]);
...
export function verifyWslProvider(distribution: string, provider: WslProvider): boolean {
  if (!distribution.trim() || !listWslDistributions().ok) return false;
  const result = runWsl(["--distribution", distribution, "--exec", "which", provider]);
  return result.status === 0;
}

export function validateWslLaunch(distribution: string, provider: WslProvider, cwd: string): string | null {
  ...
  const probe = listWslDistributions();
  ...
  if (!verifyWslProvider(distribution, provider)) return `${provider === "claude" ? "Claude Code" : "Codex"} CLI was not found inside the selected WSL distribution. Install it there, then restart Baby Menu.`;
  return null;
}
