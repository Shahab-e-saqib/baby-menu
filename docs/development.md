...
- Source mode and packaged `Baby Menu Dev` / test bundles never touch login items on macOS or Windows. Only an exact packaged production executable (`Baby Menu` on macOS or `Baby Menu.exe` on Windows) may call Electron's `setLoginItemSettings` API.
...
- See [CONTRIBUTING.md](../CONTRIBUTING.md#release-notes) for the draft-only internal-preview procedure.
